...
The `mockery` npm module is used in conjunction with `sinon` with a number of `sinon` variations to enable mocking in the unit tests.
...
`docker run -p 3000:3000 johnrellis/johnrellis-users-api:1 node app.js` where '1' is the `TRAVIS_BUILD_NUMBER`
