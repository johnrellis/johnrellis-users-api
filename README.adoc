...
The API documentation is generated using Swagger. To view the documentation, follow these steps:
...
2. Create a Swagger configuration file (swagger.json) in the root directory of the project. Here is an example configuration:
   ```json
   {
     "swagger": "2.0",
     "info": {
       "version": "1.0.0",
       "title": "johnrellis-users-api",
       "description": "An API for performing CRUDL on users"
     },
     "host": "localhost:3000",
     "basePath": "/api/v1",
     "schemes": [
...
3. Start the Swagger UI server:
   ```
   swagger-ui -f swagger.json
   ```

4. Open your browser and navigate to `http://localhost:3000/docs` to view the API documentation.
